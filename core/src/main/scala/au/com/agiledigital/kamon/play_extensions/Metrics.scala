 ...
 *
 * Supports creating new instances that categorise contexts created within them. Enables nesting traces metrics. Should
 * be used in conjunction with the [[EscapingMetricKeyGenerator]] if metric names are to be nested in statsd.
 */
trait Metrics {
...
class MetricsSingleton(category: Option[String], futureNameGenerator: FutureNameGenerator) extends Metrics {
  ...
  override def withNewContext[T](traceName: String, traceToken: Option[String], autoFinish: Boolean)(code: => T): T =
    Tracer.withNewContext(fullName(traceName), traceToken, autoFinish)(code)
